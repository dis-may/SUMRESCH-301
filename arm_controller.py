...
class Arm:
    def __init__(self, θ=np.pi/2, target=np.pi * 1.5) :
        ## VARIABLES
        self.t = 0.0    ## time
        ...
        # stimulus
        self.target = target # arbitrary target

        # motors
        self.motors = [0,0,] # (bicep_motor, tricep motor,)
                              
    def next_state_would_be(self, dt) :
        exit_time = self.t + dt
        temp_t  = self.t
        temp_θ  = self.θ
        temp_dθ = self.dθ
        ...
        while temp_t < exit_time :
            sol = solve_ivp(RHS, [temp_t, exit_time], [temp_θ, temp_dθ], args=self.motors,
                            dense_output=True, 
                            events=[
                                discontinuity_θlim1,
                                discontinuity_θlim2,
            ...
            if event_fired == 0 or event_fired == 1:
                ...
                dθ, ddθ = RHS(sol.t[-1], [temp_θ, temp_dθ], self.motors[0], self.motors[1])
    ...
    def step(self, dt) :
        next_θ, next_dθ = self.next_state_would_be(dt)
        self.t  += dt
        self.θ   = next_θ
        self.dθ  = next_dθ
        ...
        # reset the motors here
        # self.motors[0] = self.θ / np.pi / 5 # bicep
        self.motors[0] = k * (self.θ - set_point) # bicep
        self.motors[1] = -k * (self.θ - set_point) # tricep
